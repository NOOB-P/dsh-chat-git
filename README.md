...
- 历史列表里的**编辑并重新发送**：点开后用一个文本框载入**该轮完整的原始提示词**（不是卡片上被截断的展示文本），改完确认即**删掉这一轮及其之后的所有对话**并**重新发送一次**，原会话照例归档可恢复；
...
| `conversation.view` | list | 标签栏里的**历史**标签页，左右两栏：左栏是对话（每一轮带「从这里 fork」「回退到这里」「编辑并重新发送」），右栏是工作区 git（提交列表，每条带「还原到这里」） |
...
```bash
npm test              # 两套一起跑
npm run test:host     # 187 项：真实 git、假 ctx/假模型、日志折叠、整段提示词读取、间隔节流、工作区兜底、真实 loopback HTTP
npm run test:client   # 189 项：席位注册、消息→轮次反查、回退流程、三态选择器、两栏历史、编辑并重新发送、间隔选择、失败重试
```
