...
[![version](https://img.shields.io/badge/version-0.4.9-blue.svg)](#更新日志)
...
[![tests](https://img.shields.io/badge/tests-411%20passing-success.svg)](#运行测试)
...
```bash
npm pack                    # 产出 dsh-chat-git-0.4.9.tgz
dsh plugin --profile web add ./dsh-chat-git-0.4.9.tgz
```
...
- **左栏**按顺序列出该会话的每一轮，每张卡片三个动作：`从这里 fork` / `回退到这里` / `编辑并重新发送`；
...
28. **重发的模型在选择之后、会话动之前写下去。** `sessions.create` 与 `sessions.fork` 都是拿**默认选择**构造新会话的，所以 `/chat-git/set-model` 必须发生在它们**之前** —— 否则新线跑的还是旧模型，而对话框看起来一切正常。写入前先对照**实时模型目录**校验，因为一个本部署无法服务的路由要到很久以后才失败，那时对话已经搬走了。下拉框的选项是**扁平的**一份列表（provider / model 一级），而不是两个联动的选择器：这个控件在对话框里只回答一个问题。选中项会被写成**默认模型**（与 shell 自己的模型选择器一致），对话框里也如实这么写。
