...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function HistoryView(props) {
      ...
      const [confirmSha, setConfirmSha] = React.useState('')
       ...
       *
       * Restoring is armed in two steps, exactly like the per-turn icon button:
       * one click asks, the second performs. A native `confirm` is avoided
       * because it blocks the shell's own event loop.
       */
      const commitRow = (commit) => {
        const armed = confirmSha === commit.sha
        const when = clockOf(Date.parse(commit.date))
        return h('div', { key: commit.sha, className: 'dsh-chat-git-commit' },
          ...
          h('div', { className: 'dsh-chat-git-commit-body' },
            ...
            h('div', { style: panelStyles.actions },
              h('button', {
                ...
                disabled: repoBusy,
                ...(armed ? { 'data-armed': 'true' } : {}),
                onClick: () => { if (armed) void restore(commit.sha); else setConfirmSha(commit.sha) },
              }, armed ? '再点一次确认还原' : '还原到这里'),
              armed
                ? h('button', {
                  type: 'button',
                  disabled: repoBusy,
    ...
    function apply(ctx) {
       ...
       * @returns `{ ok: true, nextSessionId }` or `{ ok: false, error }`.
       */
      const resend = async ({ sessionId, turn, previous, text, cwd }) => {
