...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      '.dsh-chat-git-card{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l1,rgba(127,127,127,.28));',
      ...
      // The history cards and the confirm dialog share one button treatment, so
      // a bare `<button>` never falls back to the browser's own light-on-dark
      // default in the middle of a themed panel.
      '.dsh-chat-git-card button,.dsh-chat-git-dialog button{padding:4px 10px;font:inherit;font-size:12px;',
      ...
      '.dsh-chat-git-card button:hover:not([disabled]),.dsh-chat-git-dialog button:hover:not([disabled])',
      '{background:var(--dsw-alias-interactive-bg-hover,rgba(127,127,127,.14))}',
      '.dsh-chat-git-card button[disabled],.dsh-chat-git-dialog button[disabled]{opacity:.45;cursor:not-allowed}',
    ...
    function HistoryView(props) {
      ...
      // The dialog is opened by one of two card buttons, so its wording has to
      // follow that choice: a fork dialog never says 回退, and a rewind dialog
      // never says fork. Spelling both into every button ("仅回退/fork 对话")
      // made the user re-read the card button they had just clicked to work out
      // which of the two they were actually in.
      const scope = pending === null
        ? { conversation: '', withCode: '' }
        : pending.mode === 'fork'
          ? { conversation: '仅 fork 对话', withCode: 'fork 并还原代码' }
          : { conversation: '仅回退对话', withCode: '回退并还原代码' }
      ...
      return h('div', { style: panelStyles.root },
        h('div', { style: panelStyles.header },
          h('div', { style: { display: 'flex', alignItems: 'baseline', gap: '8px' } },
            h('h3', { style: panelStyles.title }, '历史'),
