...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function HistoryView(props) {
      ...
      /**
       * Open one turn's request in a new conversation's input box.
       *
       * The conversation move is the same fork the other two card actions use —
       * the new line ends at the turn *before* this one, so the chosen turn and
       * everything after it are gone — with one deliberate difference: nothing
       * is sent. The turn's whole request (its text and its images) is handed to
       * the new conversation's composer and the action stops there, so the user
       * keeps the last word: they can rewrite it, drop an image, change the
       * model, or decide not to send at all.
       *
       * The whole request is read from the host first, because the card only
       * carries a display clip of the prompt. That read happens before the
       * conversation moves, so a failure is reported where the user asked for
       * the action and costs nothing.
       *
        ...
        return h('div', {
          ...
          h('div', { style: panelStyles.actions },
            ...
            h('button', {
              ...
              onClick: () => { void editSend(entry) },
            }, loadingPrompt === entry.turn ? '正在读取提示词…' : '编辑并发送'),
          ),
          branchable
            ? null
      ...
      //
      // 编辑并发送 has no dialog of its own, and needs none: it moves the
      // conversation and then stops, so there is nothing to confirm. The request
      // it hands to the new composer is the user's to edit or discard, and the
      // composer's own send button is the confirmation.
      //
      ...
      const confirm = pending === null
        ? ''
        : pending.mode === 'fork' ? '确认 fork 对话' : '确认回退对话'
      ...
      const dialog = pending === null ? null : h('div', { style: panelStyles.backdrop },
        h('div', { className: 'dsh-chat-git-dialog', style: panelStyles.dialog },
          h('h4', { style: panelStyles.dialogTitle },
            `${pending.mode === 'fork' ? '从这里 fork' : '回退到这里'}`
    ...
    function apply(ctx) {
       ...
       *
       * The original is **left alone**. Archiving it here is what made the action
       * read as "this conversation rolled back to the turn before": the sidebar
       * entry the user had just clicked lost its later turns, and the fork that
       * replaced it carried the *same* title, so the two read as one conversation
       * that had moved backwards. 编辑并发送 is a fork — the original keeps every
       * turn and stays reachable, and only the new line receives the request.
       * `increaseTitle` gives that new line a distinguishable title (`… (1)`) for
       * the same reason: a fork the user cannot tell apart from its source is a
       * fork they cannot navigate back to.
       *
       ...
       * @param request - the turn, the turn before it, its whole request, and cwd.
       * @returns `{ ok: true, nextSessionId, notice }` or `{ ok: false, error }`.
       */
      const editSend = async ({ sessionId, turn, previous, text, images, cwd }) => {
        const hasPrevious = previous !== null && previous !== undefined && typeof previous.seq === 'number'
        let nextSessionId
        try {
          if (hasPrevious) {
            nextSessionId = await sessions.fork({ sessionId, atSeq: previous.seq, increaseTitle: true })
          } else {
        ...
        return {
          ok: true,
          nextSessionId,
          notice: `已新建会话并保留到第 ${String(kept)} 轮，第 ${String(turn)} 轮的提示词${carried}已放进输入区，确认后自行发送；原对话保持原样。`,
        }
      }
