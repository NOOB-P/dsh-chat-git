...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
       ...
       * and the composer's own send button is the confirmation. The original is
       * archived either way, because the conversation has genuinely moved.
      ...
      const editSend = async ({ sessionId, turn, previous, text, images, cwd }) => {
        ...
        return {
          ok: true,
          nextSessionId,
          notice: `已新建会话并保留到第 ${String(kept)} 轮，第 ${String(turn)} 轮的提示词${carried}已放进输入区，确认后自行发送。`,
        }
      }
