 ...
 *    order, each card offering 从这里 fork / 回退到这里. Registered only while
 *    the preference says so, because a tab strip is projected from the slot
 *    itself and a component cannot withdraw its own tab.
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      // The resend editor is a native textarea, so it needs the same theme
      // treatment the dialog buttons get: a bare one would be drawn white with
      // the dark theme's near-white text on it.
      '.dsh-chat-git-editor{outline:none}',
      '.dsh-chat-git-editor:focus-visible',
      '{border-color:var(--dsw-alias-brand-primary,#0f1115)!important}',
    ...
    /**
     * Flatten a model catalogue into the picker's options.
     *
     * One flat list rather than two dependent pickers, because this control sits
     * in a dialog and answers one question ("which model should this run on").
     * The key is the array index rather than `provider/model`, since a provider
     * id is an opaque string that may itself contain a separator.
     * @param catalogue - a `/chat-git/models` answer.
     * @returns `[{ key, provider, model, label }]`.
     */
    function modelRoutesOf(catalogue) {
      const routes = []
    ...
    /**
     * The picker option that matches what the deployment would use anyway.
     *
     * Seeding the control with the current route is what makes it read as
     * "change this if you want" rather than as a choice the user has to make
     * before they can resend at all.
     * @param catalogue - a `/chat-git/models` answer.
     * @param routes - the flattened options.
     * @returns the selected key, or an empty string when there is nothing to pick.
     */
    function defaultModelKey(catalogue, routes) {
      const provider = catalogue?.current?.provider
      const model = catalogue?.current?.model
      const match = routes.find((route) => route.provider === provider && route.model === model)
      return match?.key ?? routes[0]?.key ?? ''
    }
    ...
    const panelStyles = {
      ...
      // The resend editor. `font: inherit` matters here more than anywhere else
      // in this file: a bare `<textarea>` keeps the browser's own monospace
      // default and its fixed row height, which reads as a code box rather than
      // as the prompt the user is rewriting.
      editor: {
        width: '100%',
        boxSizing: 'border-box',
        font: 'inherit',
      ...
      // One labelled control row inside the dialog: the label on the left, the
      // control taking the rest of the width.
      field: {
        display: 'flex',
        alignItems: 'center',
        gap: '8px',
      },
      // A native select inside the dialog. Same reasoning as the textarea: an
      // unstyled one is drawn by the platform, which is how a dark app ends up
      // with a white popup and near-white text on it.
      picker: {
        width: '100%',
        boxSizing: 'border-box',
        font: 'inherit',
    ...
    function HistoryView(props) {
      ...
      // The text being edited for a 编辑并重新发送. Held here rather than read
      // back out of the timeline at confirm time for two reasons: the panel's
      // prompts are clipped for display, so the card cannot seed a whole
      // prompt; and the user's edit has to survive the busy round-trip.
      const [draft, setDraft] = React.useState('')
      // The turn whose whole prompt is currently being read, or 0 for none.
      // Held as a turn number rather than a flag because the read is per card:
      // a flag would relabel every card's button while one of them loaded. The
      // editor is unusable until that read lands, so an empty box can never be
      // mistaken for "the prompt was empty".
      const [loadingPrompt, setLoadingPrompt] = React.useState(0)
      // The model routes a resend may run on, flattened from the same live
      // catalogue the settings page reads. `null` means "not read yet", which is
      // what keeps the picker worded differently from an empty list: this
      // deployment having no model route is not the same fact as a read that has
      // not happened.
      const [modelRoutes, setModelRoutes] = React.useState(null)
      ...
      // because the actions that need a boundary — 编辑并重新发送 above all —
      ...
      /**
       * Open the editor on one turn: read that turn's whole prompt off the host
       * first, because the card only carries a display clip of it.
       *
       * The fork boundary this action will need — the turn *before* this one —
       * is read here too, from the list already on screen. Both reads happen
       * before the dialog opens, so a failure is reported where the user asked
       * for the action rather than in a dialog they then have to dismiss.
       */
      const edit = async (entry) => {
        ...
        // The model list is read in the same step, before the dialog opens:
        // choosing a model is part of this dialog, and a picker that populated
        // itself a moment after opening would read as a list that failed to load.
        if (modelRoutes === null) {
          const catalogue = await post('/chat-git/models', {})
          if (catalogue.ok) {
            const routes = modelRoutesOf(catalogue.value)
            setModelRoutes(routes)
            setModelKey(defaultModelKey(catalogue.value, routes))
          }
        }
        setLoadingPrompt(0)
        if (!result.ok) {
          setError(result.error?.message ?? '读取该轮提示词失败')
          return
        }
        const index = turns.findIndex((candidate) => candidate.turn === entry.turn)
        const previous = index > 0 ? turns[index - 1] : null
        setDraft(result.value.prompt)
        setPending({ entry, mode: 'resend', previous })
      }

      /** The route the picker currently has selected, or null when it has none. */
      const chosenModel = () => {
        if (modelRoutes === null || modelKey === '') return null
        const route = modelRoutes.find((candidate) => candidate.key === modelKey)
        return route === undefined ? null : { provider: route.provider, model: route.model }
      }

      /**
       * Rewrite the conversation from one turn: truncate it there and send the
       * edited prompt into the truncated session.
       *
       * The truncation is the same fork the other two card actions use, because
       * DSH has no "truncate session" API — the difference is that this one
       * carries on: the prompt is sent into the new session as a queued turn.
       * A resend that cannot be sent is reported as a failure *after* the fork,
       * because the fork already moved the conversation and saying otherwise
       * would leave the user looking at a new session with no explanation.
       */
      const resend = async (entry, previous, text) => {
        if (busy) return
        setBusy(true)
        setError('')
        const outcome = await props.resend({
          sessionId,
          turn: entry.turn,
          previous,
          text,
        ...
        setBusy(false)
        if (!outcome.ok) {
          // The editor stays open on a refusal: the one thing that can go wrong
          // here is the text itself, and closing the dialog would throw away the
          // edit the user has to fix.
          setError(outcome.error)
          return
        }
        setPending(null)
      ...
      const card = (entry, index, here) => {
        ...
        // A resend rewrites the conversation *from* this turn, so the boundary it
        // needs belongs to the turn before it. The first turn needs none: there
        // is nothing to keep, so the resend starts a fresh session in the same
        // workspace instead of forking.
        const previous = index > 0 ? turns[index - 1] : null
        const resendable = branchable && (previous === null || typeof previous.seq === 'number')
        return h('div', {
          ...
          h('div', { style: panelStyles.actions },
            ...
            h('button', {
              type: 'button',
              disabled: busy || loadingPrompt !== 0 || !resendable,
              onClick: () => { void edit(entry) },
            }, loadingPrompt === entry.turn ? '正在读取提示词…' : '编辑并重新发送'),
          ),
          branchable
            ? null
            : h('span', { style: panelStyles.muted }, '该轮没有结束序列（仍在进行中），无法定位分叉点。'),
          branchable && !resendable
      ...
      const commitRow = (commit) => {
        const when = clockOf(Date.parse(commit.date))
      ...
      // The dialog is opened by one of two card buttons, so its wording follows
      // that choice: a fork dialog never says 回退, and a rewind dialog never
      // says fork. Spelling both into every button ("仅回退/fork 对话") made the
      // user re-read the card button they had just clicked to work out which of
      // the two they were actually in.
      //
      // There is no code scope to choose any more: the repository has its own
      ...
      const confirm = pending === null
        ...
        : pending.mode === 'fork'
          ? '确认 fork 对话'
          : pending.mode === 'revert' ? '确认回退对话' : '删除并重新发送'

      // A resend is the only arm that carries an input, so it is the only one
      // whose confirm can be refused for having nothing to send.
      const resendBlocked = pending !== null && pending.mode === 'resend' && draft.trim() === ''
      ...
      const dialog = pending === null ? null : h('div', { style: panelStyles.backdrop },
        h('div', { className: 'dsh-chat-git-dialog', style: panelStyles.dialog },
          h('h4', { style: panelStyles.dialogTitle },
            `${pending.mode === 'fork' ? '从这里 fork' : pending.mode === 'revert' ? '回退到这里' : '编辑并重新发送'}`
          ...
          pending.mode === 'fork'
            ...
            : pending.mode === 'revert'
              ...
              : h('p', { style: panelStyles.dialogBody },
                  '会删掉这一轮及其之后的所有对话，然后把下面的提示词重新发送一次；当前会话随即归档（可从归档恢复）。'),
          pending.mode === 'resend'
            ? h('textarea', {
                className: 'dsh-chat-git-editor',
            ...
            : null,
          pending.mode === 'resend' ? h('div', { style: panelStyles.field },
              h('span', { style: panelStyles.muted }, '模型'),
              modelRoutes === null
                ? h('span', { style: panelStyles.muted }, '正在读取模型列表…')
                : modelRoutes.length === 0
                  ? h('span', { style: panelStyles.muted }, '这个部署没有已注册的模型路由，只能沿用当前模型。')
                  : h('select', {
                      className: 'dsh-chat-git-select',
                      style: panelStyles.picker,
                      value: modelKey,
                      disabled: busy,
                      'aria-label': '重新发送使用的模型',
                      onChange: (event) => setModelKey(event.target.value),
                    }, modelRoutes.map((route) => h('option', {
                      key: route.key,
                      value: route.key,
                    }, route.label))),
            )
          ...
          pending.mode === 'resend'
            ? h('p', { style: panelStyles.dialogBody },
                resendBlocked
                  ? '提示词不能为空。'
                  : '改动只影响这次发送，原始记录不会被改写。选择的模型会用于这次重新发送，并同时写为默认模型（与 shell 自己的模型选择器一致）。')
          ...
          h('div', { style: panelStyles.dialogActions },
            h('button', {
              ...
              onClick: () => {
                if (pending.mode === 'resend') void resend(pending.entry, pending.previous, draft)
                else void run(pending.entry, pending.mode)
              },
              ...(pending.mode === 'resend' ? { 'data-action': 'resend' } : {}),
            }, confirm),
            h('button', {
              type: 'button',
              disabled: busy,
    ...
    function apply(ctx) {
      ...
      /**
       * Deliver one prompt into a session, preferring the conversation service's
       * own send so the message is echoed and serialized exactly as a typed one.
       *
       * `ctx.conversation` is scope-addressed, so the target session's scope has
       * to be resolved first; the sessions service's binding is the fallback,
       * and it is a real fallback rather than dead code because the two are
       * separate plugins: a deployment that mounts this one without the
       * conversation controller still gets a working resend.
       * @param targetId - the session the prompt goes into.
       * @param text - the prompt body.
       * @returns `{ ok: true }` or `{ ok: false, error }`.
       */
      const sendInto = async (targetId, text) => {
        const scoped = typeof sessions.scope === 'function' ? sessions.scope(targetId) : undefined
        ...
        const session = typeof sessions.binding === 'function'
          ? sessions.binding(targetId)?.session
          : undefined
        if (session === undefined || typeof session.prompt !== 'function') {
          return { ok: false, error: 'this deployment mounts no conversation service' }
        }
        try {
          const result = await session.prompt([{ type: 'text', text }], 'queue')
          if (result !== null && typeof result === 'object' && result.ok === false) {
            return { ok: false, error: result.error?.message ?? String(result.error?.code ?? 'prompt refused') }
          }
          return { ok: true }
        } catch (error) {
          return { ok: false, error: error instanceof Error ? error.message : String(error) }
      ...
      /**
       * Rewrite the conversation from one turn and send the edited prompt again.
       *
       * This is the one card action that carries on after moving the
       * conversation. Truncating is still a fork — DSH has no "truncate session"
       * API — but the boundary belongs to the turn *before* the edited one, so
       * the edited turn and everything after it disappear from the new line.
       *
       * The first turn has no such boundary, and forking at nothing would keep
       * the whole conversation; a fresh session in the same workspace is the
       * honest expression of "nothing before this turn is kept".
       *
       * The prompt is sent *before* the original is archived, so a send that the
       * host refuses leaves the user with both lines and a message saying so,
       * rather than with an archived original and nothing sent.
       * @param request - the turn, the turn before it, the edited text, and cwd.
       * @returns `{ ok: true, nextSessionId }` or `{ ok: false, error }`.
       */
      const resend = async ({ sessionId, turn, previous, text, cwd, model }) => {
        const prompt = typeof text === 'string' ? text.trim() : ''
        if (prompt === '') return { ok: false, error: '提示词不能为空。' }

        // The model is chosen *before* the conversation moves. `session.create`
        // and `session.fork` both build the new session from the deployment's
        // default selection, so this is the only moment at which the choice can
        // still take effect — and doing it first means a refused route leaves
        // the conversation exactly where it was.
        if (model !== null && model !== undefined) {
          const applied = await post('/chat-git/set-model', {
            provider: model.provider,
            model: model.model,
          })
          if (!applied.ok) {
            return { ok: false, error: `无法使用该模型：${applied.error?.message ?? ''}` }
          }
        }
        ...
        let nextSessionId
        try {
          nextSessionId = hasPrevious
            ? await sessions.fork({ sessionId, atSeq: previous.seq })
            : await sessions.create(typeof cwd === 'string' && cwd !== '' ? { cwd } : {})
        } catch (error) {
        ...
        const sent = await sendInto(nextSessionId, prompt)
        if (!sent.ok) {
          return { ok: false, error: `对话已重建到第 ${String(hasPrevious ? previous.turn : 0)} 轮，但重新发送失败：${sent.error}` }
        }
        ...
        const sync = () => {
          if (historyState.enabled && dispose === null) {
            dispose = ctx.slots.register({
              ...
              order: 20,
              label: '历史',
            }, (props) => h(HistoryView, { ...props, branch, resend }))
