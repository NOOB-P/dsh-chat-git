 ...
 *
 * The description is the prompt that drove the turn. It falls back to the turn
 * number when no prompt was recorded — a session resumed after a restart, or a
 * turn opened without a user message. The whole line, marker included, stays
 * within {@link SUBJECT_MAX}.
 * @param turn - the turn number that just ended.
 * @param prompt - the recorded user prompt for that turn.
 * @returns the commit subject.
 */
function subjectFor(turn, prompt) {
  const room = Math.max(1, SUBJECT_MAX - SUBJECT_PREFIX.length)
  const description = clipSubject(prompt, room) || clipSubject(`第 ${String(turn)} 轮对话`, room)
  return `${SUBJECT_PREFIX}${description}`
}
...
export function apply(ctx) {
  ...
  async function checkpoint(session, turn) {
    ...
    const subject = subjectFor(turn, prompt)
      ...
      switch (route) {
        ...
        case '/chat-git/state': {
          ...
          ok(res, {
            ...
            commits: store.commitsOf(sessionId).map((entry) => ({
              sha: entry.sha,
              short: entry.short,
