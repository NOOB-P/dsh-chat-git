...
import { buildTimeline, textOfMessage, turnPrompt } from './timeline.js'
 ...
 * @param sessions - the sessions service, or undefined when it is not mounted.
 * @param sessionId - the conversation to read.
 * @returns the events, or null when the session is not live or unreadable.
 */
function eventsOf(sessions, sessionId) {
...
function timelineOf(sessions, sessionId) {
  const events = eventsOf(sessions, sessionId)
  return events === null ? null : buildTimeline(events)
}
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      switch (route) {
        ...
        case '/chat-git/turn-prompt': {
          // The whole prompt one turn was started with — what the History pane's
          // 编辑并重新发送 puts in its editor. `/chat-git/timeline` cannot carry
          // it: that answer is built for cards and clips every prompt, and
          // resending a clip would quietly ask the model something the user
          // never wrote.
          if (sessionId === '') {
            fail(res, 'bad-request', 'sessionId is required')
            return
          ...
          if (!Number.isInteger(body.turn)) {
            fail(res, 'bad-request', 'turn is required')
            return
          }
          const events = eventsOf(ctx.get('sessions'), sessionId)
          if (events === null) {
            fail(res, 'session-unavailable', 'that conversation is not loaded in this process')
            return
          }
          const prompt = turnPrompt(events, body.turn)
          if (prompt === null) {
            fail(res, 'turn-unknown', 'that turn is not in this conversation')
            return
          }
          ok(res, { turn: body.turn, prompt })
          return
        }
        ...
        case '/chat-git/repo': {
          ...
          const head = await git.head(root, signal)
          const known = store.positionOf(sessionId)
          // Where the worktree stands. A restore deliberately leaves HEAD alone,
          // so git cannot answer this after one; the store remembers it, and an
          // unknown answer resolves to HEAD — which is where a conversation that
          // has never been restored actually is. `positionKnown` keeps the two
          // apart so the pane can say which one it is showing.
          ok(res, {
            cwd,
            root,
            git: probe,
        ...
        case '/chat-git/restore': {
          ...
          // The worktree now holds that commit's content while HEAD stays where
          // it was, so the pane's 当前位置 marker has to be remembered here —
          // there is nothing in git to read it back from.
          store.rememberPosition(sessionId, sha)
        ...
        case '/chat-git/set-model': {
          // The model a resend will run on. `session.create` and `session.fork`
          // both build their child from the default selection, so choosing here
          // before either call is what makes the new line run the chosen model;
          // it is stored as the default, exactly like the shell's own picker.
          const provider = typeof body.provider === 'string' ? body.provider.trim() : ''
          const model = typeof body.model === 'string' ? body.model.trim() : ''
          if (provider === '' || model === '') {
            fail(res, 'bad-request', 'provider and model are required')
            return
          }
          const service = ctx.get('agentDefaultModel')
          if (service === undefined || typeof service.saveSelection !== 'function') {
            fail(res, 'model-unavailable', 'this deployment mounts no default-model service')
            return
          }
          // Checked against the live registry rather than stored blindly: a route
          // the deployment cannot serve would silently make the resend fail much
          // later, with the conversation already moved.
          const catalogueNow = await describeModelCatalogue()
          const listed = catalogueNow.providers.find((entry) => entry.id === provider)
          if (listed === undefined || !listed.models.some((entry) => entry.id === model)) {
            fail(res, 'unknown-model', 'that model is not registered in this deployment')
            return
          }
          try {
            await service.saveSelection({ provider, model })
          } catch (error) {
            fail(res, 'model-refused', error instanceof Error ? error.message : String(error))
            return
          }
          ok(res, { provider, model })
          return
        }
