    ...
    handler: async (req, res) => {
      ...
      switch (route) {
          ...
          ok(res, {
            ...
            turns: journal.map((entry) => {
              ...
              return {
                ...
                at: entry.at,
                prompt: entry.prompt,
                // How many images this turn's request carried. The card only
                // states the count — the durable handles themselves are read by
                // `/chat-git/turn-prompt` by the one caller that moves them.
        ...
        case '/chat-git/turn-prompt': {
          // The whole request one turn was started with — its text and its
          // images — which is what the History pane's 编辑并发送 hands to the new
          // session's input box. `/chat-git/timeline` cannot carry it: that
          // answer is built for cards and clips every prompt, and putting a clip
          // back in front of the user would quietly rewrite what they asked.
          if (sessionId === '') {
            fail(res, 'bad-request', 'sessionId is required')
            return
          }
          ...
          const request = turnPrompt(events, body.turn)
          if (request === null) {
            fail(res, 'turn-unknown', 'that turn is not in this conversation')
            return
          }
          // Text and images are separate fields because the browser half has to
          // treat them differently: the text is written into the composer, while
          // each image handle is re-read into a file through the *source*
          // session's own authorization before it can join the draft.
          ok(res, { turn: body.turn, text: request.text, images: request.images })
          return
        }
