...
/** A fresh default state. Both preferences are opt-out, so both default on. */
function emptyState() {
  return { version: 1, enabled: true, summarize: true, sessions: {} }
}
...
function normalize(raw) {
  ...
  if (typeof raw.summarize === 'boolean') state.summarize = raw.summarize
...
export function createStore(file = join(dshHome(), 'chat-git.json')) {
  ...
  return {
    ...
    /** Flip the summary preference and persist it. */
    setSummarize(summarize) {
      state.summarize = summarize === true
      flush()
      return state.summarize
    },
