 ...
 * "编辑并重新发送" hands back to the model, and resending a clip would silently
 * ask for something the user never wrote.
...
function fold(events) {
  ...
  for (const event of events) {
    ...
    if (event.type === 'user/message') {
      ...
      // card, and 编辑并重新发送 would then hand that preamble back to the model as
      // if it were the request. Skipping it leaves the turn's real prompt in place.
...
/**
 * The whole prompt one turn was started with.
 *
 * This is what the History pane's editor is seeded with and what a resend
 * sends, so it is read from the same fold the panel uses rather than kept
 * anywhere else: the session log is the only record of what was asked.
 * @param events - the session's events, in append order.
 * @param turn - the turn whose prompt is wanted.
 * @returns the unclipped prompt, or null when this log has no such turn.
 */
export function turnPrompt(events, turn) {
  if (!Array.isArray(events)) return null
  const entry = fold(events).find((candidate) => candidate.turn === turn)
  return entry === undefined ? null : entry.prompt
}
